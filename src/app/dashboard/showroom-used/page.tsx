...
import { Plus, Search, Car, Wrench, CheckCircle, AlertTriangle, X, MapPin, Package } from 'lucide-react'
...
export default function ShowroomUsedPage() {
  const { usedVehicles, addUsedVehicle, updateUsedVehicleStatus } = useShowroomStore()
  ...
  const filteredVehicles = useMemo(() => {
    if (!searchQuery) return usedVehicles
    const query = searchQuery.toLowerCase()
    return usedVehicles.filter(v =>
      v.name.toLowerCase().includes(query) ||
      v.model.toLowerCase().includes(query) ||
      v.color.toLowerCase().includes(query) ||
      v.location.toLowerCase().includes(query)
    )
  }, [usedVehicles, searchQuery])
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, newStatus: VehicleStatus) => {
    ...
    updateUsedVehicleStatus(draggingVehicle.id, newStatus)
    handleDragEnd()
  }
  ...
  // 統計情報
  const stats = useMemo(() => ({
    total: usedVehicles.length,
    onDisplay: usedVehicles.filter(v => v.status === 'on_display').length,
    totalValue: usedVehicles.filter(v => v.status !== 'sold').reduce((sum, v) => sum + v.price, 0),
    sold: usedVehicles.filter(v => v.status === 'sold').length,
  }), [usedVehicles])
      ...
      <div className="flex-shrink-0 px-6 py-4 bg-white border-b">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">中古車ステータス</h1>
              <p className="text-sm text-gray-600 mt-0.5">中古車両の状態をカンバン形式で管理</p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg w-full max-w-lg mx-4 shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold text-gray-900">中古車を追加</h2>
